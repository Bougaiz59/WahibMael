  ...
  const handleBasicSubmit = (e: React.FormEvent) => {
    ...
    if (userType === "developer") {
      ...
      setInfoPopupData({
        ...
        message:
          "Votre compte client est en cours de création. Nous configurons votre profil et vos accès pour vous permettre de publier des projets et trouver les meilleurs développeurs.",
        type: "processing",
      });
  ...
  const handleFinalSubmit = async () => {
    ...
    if (userType === "developer" && !showInfoPopup) {
      setInfoPopupData({
        ...
        message:
          "Votre profil développeur est en cours de création. Nous configurons vos compétences, votre portfolio et vos informations professionnelles pour vous connecter aux meilleurs projets.",
        type: "processing",
      });
      setShowInfoPopup(true);
    }
    ...
    const attemptSignup = async (retryCount = 0) => {
      ...
      try {
        ...
        try {
          ...
          console.log(
            "✅ Connectivité Supabase OK, session actuelle:",
            testSession
          );
        } catch (connectError) {
        ...
        if (userType === "developer") {
          ...
          const dailyRateValue = devData.daily_rate_defined
            ? devData.daily_rate
              ? parseInt(devData.daily_rate)
              : null
            : null;
          ...
          Object.assign(userMetadata, {
            ...
            experience_years: devData.experience_years
              ? parseInt(devData.experience_years)
              : 0,
        ...
        try {
          ...
          console.log(
            "❌ Erreur client Supabase, tentative API directe...",
            clientError
          );
        ...
        if (authError) {
          ...
          if (
            ...
            console.log(
              "- Recommandation: Attendre ou configurer SMTP personnalisé"
            );
          ...
          if (
            ...
            authError.name?.includes("AuthRetryableFetchError")
          ) {
            ...
            if (retryCount < maxRetries) {
              console.log(
                `🔄 Tentative de retry (${retryCount + 1}/${maxRetries})...`
              );
    ...
    try {
      ...
      setError(
        "Une erreur est survenue lors de la création du compte: " + err.message
      );
    } finally {
      setLoading(false);
      console.log(
        "🔄 FIN PROCESSUS INSCRIPTION - TIMESTAMP:",
        new Date().toISOString()
      );
    }
  };
              ...
              <form onSubmit={handleBasicSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <label
                      className={`cursor-pointer rounded-lg border-2 p-6 text-center transition-all hover:border-black ${
                        userType === "client"
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          setUserType(e.target.value as "client")
                        }
                        className="sr-only"
                      />
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          setUserType(e.target.value as "developer")
                        }
                        className="sr-only"
                      />
                      <div className="text-3xl mb-3">💻</div>
                      <div className="font-bold text-black text-lg">
                        {t("signup.developer.title")}
                      </div>
                      <div className="text-sm text-gray-600 mt-1">
                        {t("signup.developer.description")}
                      </div>
                    </label>
                  </div>
                </div>
              ...
              <div className="space-y-6">
                <div className="bg-black rounded-lg p-4 border-2 border-black">
                  <h3 className="font-bold text-white mb-2">
                    {t("signup.developer.profile")}
                  </h3>
                  <p className="text-gray-300 text-sm">
                    Ces informations aideront les clients à vous trouver et à
                ...
                <div>
                  ...
                  <Input
                    ...
                    value={devData.title}
                    onChange={(e) =>
                      setDevData((prev) => ({ ...prev, title: e.target.value }))
                    }
                ...
                <div>
                  ...
                  <textarea
                    value={devData.bio}
                    onChange={(e) =>
                      setDevData((prev) => ({ ...prev, bio: e.target.value }))
                    }
                    ...
                    <div className="mb-3">
                      <label className="flex items-center space-x-2 cursor-pointer">
                        <input
                          ...
                          onChange={(e) =>
                            handleDailyRateDefinedChange(!e.target.checked)
                          }
                ...
                <div className="flex space-x-4">
                  ...
                  >
                    {loading
                      ? t("signup.creating")
                      : t("signup.create.developer.profile")}
                  </button>
                </div>
              </div>
            )}
