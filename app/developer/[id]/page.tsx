...
export default function DeveloperProfilePage() {
    ...
    try {
      ...
      if (ratingsData && ratingsData.length > 0) {
        ...
        const ratingsWithNames = ratingsData.map((rating) => ({
          ...rating,
          client_name:
            clientsData?.find((client) => client.id === rating.client_id)
              ?.full_name || "Client anonyme",
        }));
  ...
  const loadDeveloperProfile = async () => {
    try {
      ...
      const combinedProfile = {
        ...
        years_of_experience:
          devProfile?.experience_years || devProfile?.years_of_experience,
        ...
        average_rating: useCalculatedStats
          ? Math.round(calculatedAverage * 10) / 10
          : devProfile?.average_rating || 0,
        total_ratings: useCalculatedStats
          ? calculatedTotal
          : devProfile?.total_ratings || 0,
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-600 mb-4">
            {error || t("profile.developer.not.found")}
          </p>
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
            ...
            <div className="lg:w-1/3">
              <div className="bg-white rounded-2xl p-8 border-2 border-gray-200 hover:border-gray-300 transition-all duration-300">
                <div className="text-center mb-6">
                  ...
                  <div
                    className={`w-32 h-32 rounded-full mx-auto mb-4 bg-gray-200 flex items-center justify-center border-4 border-gray-300 ${
                      developer.profile_image &&
                      developer.profile_image.trim() !== ""
                        ? "hidden"
                        : ""
                  ...
                  {developer.hourly_rate && (
                    <div className="text-center mb-4">
                      <div className="text-2xl font-black text-black">
                        {developer.hourly_rate}€
                        {t("profile.developer.daily.rate")}
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-3 border-t border-gray-200 pt-6">
                  <h3 className="font-black text-lg text-black mb-4">
                    {t("profile.developer.contact.info")}
                  </h3>
                  ...
                  {developer.years_of_experience && (
                    <div className="flex items-center text-gray-600">
                      ...
                      <span>
                        {developer.years_of_experience}{" "}
                        {t("profile.developer.experience.years")}
                      </span>
                    </div>
                  )}
              ...
              <div className="bg-white rounded-2xl p-8 border-2 border-gray-200 hover:border-gray-300 transition-all duration-300">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xl font-black text-black">
                    {t("profile.developer.ratings.reviews")}
                  </h2>
                  <button
                    onClick={() => {
                      loadDeveloperProfile();
                ...
                {ratings.length > 0 ? (
                  <div className="mb-6">
                    ...
                    {developer.average_rating > 0 &&
                      developer.total_ratings > 0 && (
                        <div className="flex items-center space-x-4 mb-4">
                          ...
                          <div>
                            ...
                            <p className="text-gray-600 text-sm">
                              {developer.total_ratings}{" "}
                              {t("profile.developer.ratings.total")}
                            </p>
                          </div>
                        </div>
                      )}
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center space-x-2">
                              ...
                              <span className="text-sm text-gray-600">
                                {t("profile.developer.ratings.by")}{" "}
                                {rating.client_name ||
                                  t("profile.developer.ratings.anonymous")}
                              </span>
                            </div>
                          ...
                          {rating.project_title && (
                            <p className="text-sm font-semibold text-gray-700 mb-1">
                              {t("profile.developer.ratings.project")}:{" "}
                              {rating.project_title}
                            </p>
                          )}
      ...
      {showContactModal && developer && currentUser && (
        <ContactModal
